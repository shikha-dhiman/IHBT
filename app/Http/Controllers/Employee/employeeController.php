...
class employeeController extends Controller
{
    ...
    {
        $user = User::where('user_type', $req->input('type'))->orderBy('id', 'DESC')->get();
        ...
        if($req->isMethod('post')){
           ...
           if($validator->fails()){
                ...
                if($check_if_exist == TRUE){
                    ...
                    $user->age = $req->age;
                    $user->sex = $req->gender;
                    $user->regular_medicine = $req->regular_medicine;
                    $user->pre_existing_condition_disease = $req->pre_existing_condition_disease;
                    $user->save();
                    $url =$req->url;
                    if($req->input('type') == 1){
                        $url = 'employees';
                    ...
                    return redirect('owner/'.$url.'/?type='.$req->input('type'))->with('success', 'Added successfully.');
                }
            }
        }
         ...
         if($req->isMethod('post')){
           ...
           if($validator->fails()){
                ...
                if($req->file()) {
                    ...
                    'regular_medicine' =>$req->regular_medicine,
                    'pre_existing_condition_disease' =>$req->pre_existing_condition_disease,
                    ];
                }else{
                    $params = ['name' => $req->name,
                    'mobile' => $req->mobile,
                ...
                return redirect('owner/'.$url.'/?type='.$req->type)->with('success', 'Updated successfully.');
            }
        }
        $user = User::where('id', $req->id)->get();
        return view('Owner.Employee.edit', compact('user'));
    }
     ...
     public function card(Request $req)
    {
        ...
        return view('Owner.Employee.card', compact('user', 'family_member'));
    }
     public function generate_pdf(Request $req)
    {
      ...
      $pdf = PDF::loadView('Owner.Employee.pdf', ['user' => $user, 'family_member' => $family_member]);
     ...
     public function detail(Request $req)
    {
        ...
        return view('Owner.Employee.detail', compact('family_member', 'employee', 'noOfMember'));
    }
}
